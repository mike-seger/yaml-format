	...
	private void addCollapsedSubTree(
		...
		if (aliasValueMap.containsKey(node.value)) {
			...
			if (childTree.size() == 1) {
				addCollapsedSubTree(
						parentTree,
						new Node(node.key, (childTree.entrySet().iterator().next())),
						aliasValueMap);
				newNode = null; // do not add this node to parent
			} else {
				var newChildTree = new TreeMap<String, Object>();
				childTree.forEach(
						(key, value) -> addCollapsedSubTree(newChildTree, new Node(key, value), aliasValueMap));
	...
	private void addMappedSubTreeKeys(Map<String, Object> parentTree, Node node) {
		if (node.key.contains(".")) {
			...
			for (int i = 0; i < keys.length - 1; i++) {
				parentTree =
						(Map<String, Object>) addNodeToTree(parentTree, keys[i], new HashMap<String, Object>());
			}
		}
